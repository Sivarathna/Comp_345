...
private:
	...
	int uranium[17];
	int resourceBank[4];
	vector<PowerPlant> availablePowerPlants;
public:
	Market();
	...
	void purchasePowerPlant(Player& player, int index, int cost);
	void replenishPowerPlants(vector<PowerPlant>& remainingPowerPlants);
	vector<PowerPlant>& getAvailablePowerPlants();

	void BidPowerPlants(vector<Player>& players);
};